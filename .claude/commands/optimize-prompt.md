...
**ALWAYS use TodoWrite** to show progress to the user:

```
TodoWrite([
  { content: "Analyze user's original request", status: "in_progress", activeForm: "Analyzing original request" },
  { content: "Identify improvements needed", status: "pending", activeForm: "Identifying improvements" },
  { content: "Apply optimization framework", status: "pending", activeForm: "Applying optimization framework" },
  { content: "Generate compact optimized prompt", status: "pending", activeForm: "Generating optimized prompt" }
])
```
...
5. **ALWAYS USE TodoWrite** - Track progress so user can see each optimization step
6. **ALWAYS INCLUDE SKILL IN PROMPT** - Prepend the recommended skill directly in the optimized prompt
7. **ANALYZE INTENT, NOT KEYWORDS** - Determine what the user actually wants to achieve
8. **KEEP OUTPUT COMPACT** - MAX 5-7 lines, inline lists, no excessive line breaks, easy to copy
